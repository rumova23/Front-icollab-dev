...
export class IncidentService {
	...
	listById(id): Observable<IncidentOutDTO> {
		this.parameters = this.globalService.setXTenantId_Plant();
		return this.http.get<IncidentOutDTO>(
			`${EPs.incident.get}${id}`
			, { params: this.parameters }
	...
	saveIncident(incidentInDTO: IncidentDTO): Observable<IncidentOutDTO> {
		this.parameters = this.globalService.setXTenantId_Plant();
	...
	upload(idTypeConfig: number, fileObj) {
		...
		if (idTypeConfig === 3) {
			...
			//return this.http.post(`${EPs.incident.}configuration/indicator/saveFile`, fileObj, { params: this.parameters });
		}
	}
	...
	downloadFile(idTypeConfig: number, fileId: number) {
		...
		if (idTypeConfig === 3) {
			...
			//return this.http.get<Blob>(`${EPs.incident.}configuration/indicator/downloadFile/` + fileId, { params: this.parameters, responseType: 'blob' as 'json' });
		}
	}
	getDocuments(idTypeConfig: number, id: number, typeDocument: string): Observable<any> {
	  ...
	  if (idTypeConfig === 3) {
		...
		//return this.http.get(`${EPs.incident.}configuration/indicator/listFiles/${id}`, {params : this.parameters });
	  }
	}
	...
	deleteFile(idTypeConfig: number, id): Observable<any> {
		...
		if (idTypeConfig === 3) {
		  ...
		  //return this.http.delete( `${ EPs.incident. }configuration/indicator/deleteFile/` + id, {params : this.parameters });
		}
	  }
