export const data_graficaDosa = [
    {
        backgroundColor:'rgba(51, 122, 32, 0.3)',
        borderColor:'rgba(51, 122, 32, 1)',
        fill : false,
        label:"A condiciones despues de outage",
        data:[
            ,{x:0,      y: 561.32}
            ,{x:5,      y: 561.32}
            ,{x:10,     y: 561.32}
            ,{x:12.9,   y: 561.32}
            ,{x:15,     y: 555.33}
            ,{x:20,     y: 543.09}
            ,{x:24.7,   y: 531.19}
            ,{x:30,     y: 517.88}
            ,{x:33,     y: 510.59}
            ,{x:34.8,   y: 505.60}
            ,{x:40,     y: 492.67}
            ,{x:45,     y: 480.07}
            ,{x:50,     y: 467.47}
        ],
        yAxisID: 'y-axis-1'
    },
    {
        backgroundColor:'rgba(0, 174, 255, 0.3)',
        borderColor:'rgba(0, 174, 255, 1)',
        fill : false,
        label:"Potencia Real Domostrada (MW)",
        data:[
            ,{x:0,      y: 552.476}
            ,{x:5,      y: 552.476}
            ,{x:10,     y: 552.476}
            ,{x:12.9,   y: 552.476}
            ,{x:15,     y: 546.588}
            ,{x:20,     y: 534.538}
            ,{x:24.7,   y: 522.828}
            ,{x:30,     y: 509.727}
            ,{x:33,     y: 502.553}
            ,{x:34.8,   y: 497.633}
            ,{x:40,     y: 484.916}
            ,{x:45,     y: 472.510}
            ,{x:50,     y: 460.105}
        ],
        yAxisID: 'y-axis-1'
    },
    {
        backgroundColor:'rgba(242, 203, 5,  1)',
        borderColor:'rgba(242, 203, 5,  1)',
        fill : false,
        label:"Con F4 Garantizado",
        data:[
            ,{x:0,      y: 577.3159982}
            ,{x:5,      y: 577.3159982}
            ,{x:10,     y: 577.3159982}
            ,{x:12.9,   y: 573.3908737}
            ,{x:15,     y: 569.4657491}
            ,{x:20,     y: 556.0211992}
            ,{x:24.7,   y: 538.8525967}
            ,{x:30,     y: 519.5869819}
            ,{x:33,     y: 510.1056169}
            ,{x:34.8,   y: 500.6242518}
            ,{x:40,     y: 484.916}
            ,{x:45,     y: 472.5105}
            ,{x:50 ,    y: 460.105}
        ],
        yAxisID: 'y-axis-1'
    },
    {
        backgroundColor:'rgba(196, 88, 80, 1)',
        borderColor:'rgba(196, 88, 80, 1)',
        fill : false,
        label:"PPA (MW)",
        pointHitRadius: 20,
        data:[
            ,{x:0,      y: 523.55}
            ,{x:5,      y: 527.38}
            ,{x:10,     y: 530.98}
            ,{x:12.9,   y: 533.04}
            ,{x:15,     y: 529.03}
            ,{x:20,     y: 520.29}
            ,{x:24.7,   y: 512.84}
            ,{x:30,     y: 504.16}
            ,{x:33,     y: 498.71}
            ,{x:34.8,   y: 495}
            ,{x:40,     y: 478.75}
            ,{x:45,     y: 460.77}
            ,{x:50,     y: 442.79}
        ],
        yAxisID: 'y-axis-1'
    },
    {
        backgroundColor:'rgba(75, 192, 192, 0.3)',
        borderColor:'rgba(75, 192, 192, 1)',
        label:"Capacidad Excedente (MW) actual F4",
        fill: true,
        pointHitRadius: 0,
        data:[
            ,{x:0,      y: 37.77}
            ,{x:5,      y: 33.94}
            ,{x:10,     y: 30.34}
            ,{x:12.9,   y: 28.28}
            ,{x:15,     y: 26.3}
            ,{x:20,     y: 22.8}
            ,{x:24.7,   y: 18.35}
            ,{x:30,     y: 13.72}
            ,{x:33,     y: 11.88}
            ,{x:34.8,   y: 10.6}
            ,{x:40,     y: 13.92}
            ,{x:45,     y: 19.3}
            ,{x:50,     y: 24.68}
        ],
        yAxisID: 'y-axis-2'
    }
];