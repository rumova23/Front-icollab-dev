...
@Component({
  selector: 'app-mining-if-c-water',
  templateUrl: './mining-if-c-water.component.html',
  styleUrls: ['./mining-if-c-water.component.scss']
})
export class MiningIFCWaterComponent implements OnInit {
	...
	onBtnSaveUpdate(){
		...
		debugger;
		this.indicesService.waterCostIncreaseSaveAndEdit(myyear,mymonth,salary).subscribe(data=>{
	...
	setData(data){
		...
		this.tableData01 = [
			{order:1,fechaOp:this.datePipe.transform(new Date(data.dateOp) , 'MM/yyyy'),index:'ISN',date:this.datePipe.transform(new Date(data.dateReference) , 'dd/MM/yyyy'),pcam:data.value}
		];
		let i = 0;
