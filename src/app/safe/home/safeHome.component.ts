...
export class SafeHomeComponent implements OnInit {
	...
	ngAfterViewInit() {
		//this.viewContainerRef.createComponent(this.componentFactoryResolver.resolveComponentFactory(MiningIFISalaryincreaseComponent));/*
		...
		this.subscriptions.push(this.eventService.onChangePage.subscribe({
			next: (event: EventMessage) => {
				...
				switch (event.descriptor) {
					...
					case 'Safe.SafePpaMonitoringProfileStationComponent':
						banderaTemporal = true;
						this.viewContainerRef.createComponent(
							this.componentFactoryResolver.resolveComponentFactory(
								SafePpaMonitoringProfileStationComponent
